  ...
  try {
    ...
    if (e.postData && e.postData.contents) {
      try {
        ...
        // Not JSON, check for form data
        data = null;
      }
    }
    
    // If not JSON, try form data (for form submissions)
    if (!data && e.parameter && e.parameter.data) {
      try {
        data = JSON.parse(e.parameter.data);
      } catch (formError) {
        throw new Error('Could not parse data from request');
      }
    }
    ...
    // Return HTML response (works better for form submissions)
    return HtmlService.createHtmlOutput(
      '<html><body><h2>Success!</h2><p>Your expert responses have been recorded.</p><p>You may close this window.</p><script>setTimeout(function(){window.close();},2000);</script></body></html>'
    );
      
  } catch (error) {
    console.error('Error:', error);
    return HtmlService.createHtmlOutput(
      '<html><body><h2>Error</h2><p>' + error.toString() + '</p></body></html>'
    );
  }
}
